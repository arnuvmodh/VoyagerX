...
@Autonomous
public class AutoAbstractionTest extends LinearOpMode {
        ...
        while (opModeIsActive()) {
            switch (curState) {
                ...
                case traj2:
                    ...
                    if (timer.seconds() > 1.2 && timer.seconds() < 1.9) {
                        intakeGrab(0.285, timer, 1.2, 1.6);
                    }
                    if (oneTimeSwitch[2] && timer.seconds() > 1.9) {
                        horizontalSlidePosition = 0;
                        oneTimeSwitch[2] = false;
                    }
                    if(timer.seconds()>2&&timer.seconds()<3.2) {
                        intakeFlipIn();
                    }
                    if (timer.seconds() > 3.2 && timer.seconds() < 3.3) {
                        outtakeGrab();
                    }
                    if (timer.seconds() > 3.3 && timer.seconds() < 3.6) {
                        intakeLetGo();
                    }
                ...
                case traj4:
                    ...
                    if (timer.seconds() > 1.5 && timer.seconds() < 2.2) {
                        intakeGrab(0.285, timer, 1.5, 2.2);
                    }
    ...
    void intakeGrab(double startingPosition, ElapsedTime timer, double startTime, double finishTime) {
        horizontalSlidePosition=startingPosition+(1-startingPosition)*((startTime-timer.seconds())/(startTime-finishTime));
        robot.spintake.spinIn(1);
    }
