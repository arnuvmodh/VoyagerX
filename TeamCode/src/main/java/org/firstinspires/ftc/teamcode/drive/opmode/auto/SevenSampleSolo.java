...
@Autonomous
public class SevenSampleSolo extends LinearOpMode {
    ...
    enum State {
        ...
        thirdSubmersibleIntake,
        thirdSubmersibleOuttake,
        idle
    }
    ...
    public void runOpMode() throws InterruptedException {
        ...
        SubmersibleUI thirdSubmersibleSample = new SubmersibleUI(2);
        ...
        while(!isStopRequested()) {
            if(thirdSubmersibleSample.userInput(gamepad1, telemetry)) break;
        }
        while(!isStopRequested()) {
            telemetry.addLine("Press Share to Continue");
            telemetry.update();
        ...
        Trajectory secondSampleIntake = drive.trajectoryBuilder(firstSampleOuttake.end())
                .lineToSplineHeading(new Pose2d(-19.5, 14.5176, 1.5766))
                .build();
        ...
        Trajectory secondSubmersibleOuttake = drive.trajectoryBuilder(secondSubmersibleIntake.end(), true)
                .splineToLinearHeading(new Pose2d(-18.25, 6.2, 0.75), Math.toRadians(-180))
                .build();
        Trajectory thirdSubmersibleIntake = drive.trajectoryBuilder(secondSubmersibleOuttake.end())
                .splineToLinearHeading(thirdSubmersibleSample.getTrajectory(),0)
                .build();
        Trajectory thirdSubmersibleOuttake = drive.trajectoryBuilder(thirdSubmersibleIntake.end(), true)
                .splineToLinearHeading(new Pose2d(-18.25, 6.2, 0.75), Math.toRadians(-180))
                .build();
        ...
        boolean seventhIntakeSuccess = false;
        boolean seventhPositionReached = false;
        ...
        while (opModeIsActive()) {
            ...
            switch (curState) {
                ...
                case thirdSampleIntake:
                    ...
                    if(timer.seconds()>1.7&&timer.seconds()<2.3) {
                        intakeFlipIn();
                    }
                    if (timer.seconds() > 2.3 && timer.seconds() < 2.5) {
                        outtakeGrab();
                    }
                    if (timer.seconds() > 2.5 && timer.seconds() < 2.6) {
                        intakeLetGo();
                    }
                        ...
                        if(!fifthIntakeSuccess) {
                            ...
                            if(timer.seconds() > 0.5 && timer.seconds() < 1) {
                                ...
                                drive.followTrajectoryAsync(drive.trajectoryBuilder(currentPose)
                                        .lineToSplineHeading(firstSubmersibleSample.getPostSweepTrajectory())
                                        .build());
                            }
                            ...
                            if(timer.seconds() > 1 && timer.seconds() < 2) {
                                Pose2d currentPose = drive.getPoseEstimate();
                                drive.followTrajectoryAsync(drive.trajectoryBuilder(currentPose)
                                        .lineToSplineHeading(firstSubmersibleSample.getTrajectory())
                                        .build());
                            }
                            ...
                            if(timer.seconds() > 2.5 && timer.seconds() < 3) {
                                if(oneTimeSwitch[98]) {
                                    ...
                                    firstSubmersibleSample.offsetX();
                                    drive.followTrajectoryAsync(drive.trajectoryBuilder(currentPose)
                                            .lineToSplineHeading(firstSubmersibleSample.getTrajectory())
                                            .build());
                    ...
                    if(sixthPositionReached) {
                        if(!sixthIntakeSuccess) {
                            ...
                            if(timer.seconds() > 0.5 && timer.seconds() < 1) {
                                Pose2d currentPose = drive.getPoseEstimate();
                                drive.followTrajectoryAsync(drive.trajectoryBuilder(currentPose)
                                        .lineToSplineHeading(secondSubmersibleSample.getPostSweepTrajectory())
                                        .build());
                            }
                            ...
                            if(timer.seconds() > 1 && timer.seconds() < 1.3) {
                                Pose2d currentPose = drive.getPoseEstimate();
                                drive.followTrajectoryAsync(drive.trajectoryBuilder(currentPose)
                                        .lineToSplineHeading(secondSubmersibleSample.getTrajectory())
                                        .build());
                            }
                            ...
                            if(timer.seconds() > 2.5 && timer.seconds() < 3) {
                                ...
                                if(oneTimeSwitch[97]) {
                                    ...
                                    secondSubmersibleSample.offsetX();
                                    drive.followTrajectoryAsync(drive.trajectoryBuilder(currentPose)
                                            .lineToSplineHeading(secondSubmersibleSample.getTrajectory())
                                            .build());
                                    oneTimeSwitch[97] = false;
                                }
                                intakeFlipIn();
                                horizontalSlidePosition=0;
                                robot.clawPivot.flipTo(0.5);
                ...
                case secondSubmersibleOuttake:
                    ...
                    if (!drive.isBusy() && timer.seconds() > 2.7) {
                        raiseVertSlides = false;
                        drive.followTrajectoryAsync(thirdSubmersibleIntake);
                        curState = State.thirdSubmersibleIntake;
                        timer.reset();
                    }
                    break;
                case thirdSubmersibleIntake:
                    if(timer.seconds() > 2 && !seventhIntakeSuccess) {
                        timer.reset();
                        seventhPositionReached = true;
                    }
                    if(seventhPositionReached) {
                        if(!seventhIntakeSuccess) {
                            if(robot.colorSensor.getColorAsString().equals(oppositeAllianceColor)) {
                                robot.spintake.spinOut(1);
                                break;
                            }
                            ...
                            if(timer.seconds() > 0.5 && timer.seconds() < 1) {
                                Pose2d currentPose = drive.getPoseEstimate();
                                drive.followTrajectoryAsync(drive.trajectoryBuilder(currentPose)
                                        .lineToSplineHeading(thirdSubmersibleSample.getPostSweepTrajectory())
                                        .build());
                            }
                            ...
                            if(timer.seconds() > 1 && timer.seconds() < 1.3) {
                                Pose2d currentPose = drive.getPoseEstimate();
                                drive.followTrajectoryAsync(drive.trajectoryBuilder(currentPose)
                                        .lineToSplineHeading(thirdSubmersibleSample.getTrajectory())
                                        .build());
                            }
                            ...
                            if (!drive.isBusy() && timer.seconds() > 0.3) {
                                ...
                                horizontalSlidePosition = 0;
                                robot.clawPivot.flipTo(0.5);
                                thirdSubmersibleOuttake = drive.trajectoryBuilder(secondSubmersibleIntake.end(), true)
                                        .splineToLinearHeading(new Pose2d(-18.25, 6.2, 0.75), Math.toRadians(-180))
                                        .build();
                                drive.followTrajectoryAsync(thirdSubmersibleOuttake);
                                curState = State.thirdSubmersibleOuttake;
                                timer.reset();
                            }
                        }
                    }
                    break;
                case thirdSubmersibleOuttake:
                    if (timer.seconds() > 0.5 && timer.seconds() < 0.7) {
                        outtakeGrab();
                    }
                    if (timer.seconds() > 0.7 && timer.seconds() < 0.9) {
